...
/**
 * Created by nankai on 06/13/2016.
 *
 * @author nankai
 */
...
//                    }
//                }
                // route the request through the custom URL layout_loading method
                view.loadUrl(url);
            ...
            public void onUnhandledInputEvent(WebView view, InputEvent event) {
                if (Build.VERSION.SDK_INT >= 21) {
                    if (mCustomWebViewClient != null) {
                        mCustomWebViewClient.onUnhandledInputEvent onUnhandledInputEvent(view, event);
                    } else {
    ...
    /**
     * Handles a download by layout_loading the file from `fromUrl` and saving it to `toFilename` on the external storage
     * <p/>
     ...
     * @param fromUrl    the URL of the file to download, e.g. the one from `VnApnicWebView.onDownloadRequested(...)`
