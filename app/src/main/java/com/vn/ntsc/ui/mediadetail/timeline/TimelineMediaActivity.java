...
public class TimelineMediaActivity extends BaseMediaActivity<MediaDetailPresenter> implements MediaDetailContract.View {
    ...
    public void onCreateView(View rootView) {
        ...
        getBundleData();
        setupViewPager(mViewPager, convertTimelineDataToMediaData(), mFirstShowIndex);
    ...
    public void animateAlphaWhenSwipeImage(float percent) {
        ...
        bottomPanel.animate().alpha(0f).setDuration(0).start();
    }
    ...
    public void animateAlphaFinish1F() {
        ...
        bottomPanel.animate().alpha(1f).setDuration(0).start();
    }
    ...
    public void onPageSelected(int position) {
        ...
        ListBuzzChild currentBuzz = getCurrentBuzz();

        if (currentBuzz != null) {
            updateBottomView(getCurrentBuzz());

            //http://10.64.100.201/issues/11487
            // Kiểm tra nếu là video hoặc là liveStream(đã kết thúc) thì mới tăng
            if (currentBuzz.buzzType.equals(TypeView.MediaDetailType.VIDEO_TYPE) ||
                    (currentBuzz.buzzType.equals(TypeView.MediaDetailType.STREAM_TYPE) && currentBuzz.streamStatus.equals(Constants.LIVE_STREAM_OFF))) {
                LogUtils.e(TAG, "start requestIncreaseViewNumberOfVideo");
                requestIncreaseViewNumberOfVideo();
            }
        }
    }
    ...
    /* Request increase the number of view video*/
    private void requestIncreaseViewNumberOfVideo() {
        IncreaseNumberViewVideoRequest request = new IncreaseNumberViewVideoRequest(UserPreferences.getInstance().getToken(), getCurrentBuzz().buzzId);
        getPresenter().increaseViewNumberOfVideo(request);
    }
    ...
    /*Remove item list*/
        getCurrentMedia().onActivityDestroy();
