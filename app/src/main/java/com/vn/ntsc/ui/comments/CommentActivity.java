...
import com.vn.ntsc.ui.comments.helper.CommentHeaderHelper;
import com.vn.ntsc.ui.comments.helper.LiveStreamTimelineHelper;
import com.vn.ntsc.ui.comments.helper.MediaTimelineFiveBuzzHelper;
import com.vn.ntsc.ui.comments.helper.MediaTimelineFourBuzzHelper;
import com.vn.ntsc.ui.comments.helper.MediaTimelineMoreBuzzHelper;
import com.vn.ntsc.ui.comments.helper.MediaTimelineOneBuzzHelper;
import com.vn.ntsc.ui.comments.helper.MediaTimelineThreeBuzzHelper;
import com.vn.ntsc.ui.comments.helper.MediaTimelineTwoBuzzHelper;
import com.vn.ntsc.ui.comments.helper.ShareLiveStreamTimelineHelper;
import com.vn.ntsc.ui.comments.helper.ShareTimelineHelper;
import com.vn.ntsc.ui.comments.helper.VideoAudioTimelineHelper;
...
public class CommentActivity extends BaseActivity<CommentPresenter> implements CommentContract.View,
        ...
        SwipeRefreshLayout.OnRefreshListener, CommentAdapterListener, HeaderCommentListener {
    ...
    private CommentHeaderHelper headerViewHolder;
            ...
            switch (socketEvent.getMessage().getMessageType()) {
                case Constants.SOCKET_BUZZ_CMT:
                    ...
                    if (listCommentBean.buzzId.equals(this.buzzID)) {
                        ...
                        headerViewHolder.updateCommentNumber(listCommentBean);
                        headerViewHolder.notifyDataSetChanged();
                    ...
                    if (buzzSubCommentBean.buzzId.equals(this.buzzID)) {
                        for (ListCommentBean bean : adapter.getData()) {
                            if (buzzSubCommentBean.cmtId.equals(bean.cmtId)) {
                                ...
                                headerViewHolder.updateCommentNumber(buzzSubCommentBean);
                                headerViewHolder.notifyDataSetChanged();
    ...
    public void onViewMore(ListCommentBean itemBean, int position) {
        LogUtils.i("Comment", "position(3): " + position);
    ...
    private void updateSubComment(final ListCommentBean commentBean) {
        ...
        for (int i = 0; i < adapter.getData().size(); i++) {
            ...
            if (adapter.getData().get(i).cmtId.equals(commentBean.cmtId)) {
                ...
                headerViewHolder.updateCommentNumber(commentBean);
                headerViewHolder.notifyDataSetChanged();
    ...
    private void updateView(BuzzBean itemBuzzBean) {
        ...
        headerViewHolder.notifyItemSetChanged(itemBuzzBean);
    }

    //----------------------------------------------------------------
    //---------------- Header timeline detail ------------------------
    //----------------------------------------------------------------
    @SuppressLint({"SwitchIntDef", "InflateParams"})
    private View initLayoutHeader(BuzzBean itemBuzzBean) {

        final @TimelineType int viewType = getTypeView(itemBuzzBean);

        LayoutInflater inflater = LayoutInflater.from(this);
        switch (viewType) {
            //Buzz share
            case TimelineType.BUZZ_TYPE_SHARE_AUDIO:
                headerViewHolder = new ShareTimelineHelper(inflater.inflate(R.layout.layout_item_timeline_share_audio, null), viewType);
                break;

            case TimelineType.BUZZ_TYPE_SHARE_LIVE_STREAM:
                headerViewHolder = new ShareLiveStreamTimelineHelper(inflater.inflate(R.layout.layout_item_timeline_share_live_stream, null), viewType);
                break;
            //Buzz normal
            case TimelineType.BUZZ_TYPE_MULTI_BUZZ_VIDEO_1:
            case TimelineType.BUZZ_TYPE_MULTI_BUZZ_AUDIO:
                headerViewHolder = new VideoAudioTimelineHelper(inflater.inflate(R.layout.layout_item_timeline_video_1, null), viewType);
                break;
            case TimelineType.BUZZ_TYPE_MULTI_BUZZ_LIVE_STREAM:
                headerViewHolder = new LiveStreamTimelineHelper(inflater.inflate(R.layout.layout_item_timeline_live_stream_1, null), viewType);
                break;
            case TimelineType.BUZZ_TYPE_MULTI_BUZZ_IMAGE_1:
                headerViewHolder = new MediaTimelineOneBuzzHelper(inflater.inflate(R.layout.layout_item_timeline_image_1, null), viewType);
                break;
            case TimelineType.BUZZ_TYPE_MULTI_BUZZ_2:
                headerViewHolder = new MediaTimelineTwoBuzzHelper(inflater.inflate(R.layout.layout_item_timeline_image_2, null), viewType);
                break;
            case TimelineType.BUZZ_TYPE_MULTI_BUZZ_3:
                headerViewHolder = new MediaTimelineThreeBuzzHelper(inflater.inflate(R.layout.layout_item_timeline_image_3, null), viewType);
                break;
            case TimelineType.BUZZ_TYPE_MULTI_BUZZ_4:
                headerViewHolder = new MediaTimelineFourBuzzHelper(inflater.inflate(R.layout.layout_item_timeline_image_4, null), viewType);
                break;
            case TimelineType.BUZZ_TYPE_MULTI_BUZZ_5:
                headerViewHolder = new MediaTimelineFiveBuzzHelper(inflater.inflate(R.layout.layout_item_timeline_image_5, null), viewType);
                break;
            case TimelineType.BUZZ_TYPE_MULTI_BUZZ_MORE:
                headerViewHolder = new MediaTimelineMoreBuzzHelper(inflater.inflate(R.layout.layout_item_timeline_image_more, null), viewType);
                break;
            default:
                headerViewHolder = new CommentHeaderHelper(inflater.inflate(R.layout.layout_item_timeline_status, null), viewType);
                break;
        }

        headerViewHolder.setListener(this);
        headerViewHolder.onBindView(itemBuzzBean);
        return headerViewHolder.getItemView();
    }
    ...
    int getTypeView(BuzzBean itemBuzzBean) {
        if (itemBuzzBean.isBuzzShare()) {
            ...
            if (childNumber == 1) {
                switch (itemBuzzBean.listChildBuzzes.get(0).buzzType) {
    ...
    private void setObserverEventBus() {
        RxEventBus.subscribe(SubjectCode.SUBJECT_UPDATE_TIMELINE_LIKE, this, new Consumer<Object>() {
            ...
            @Override
            public void accept(Object buzzId) {
                if (buzzId != null) {
        ...
        RxEventBus.subscribe(SubjectCode.SUBJECT_UPDATE_TIMELINE_SHARE, this, new Consumer<Object>() {
            ...
            public void accept(Object buzzId) {
                if (buzzId != null) {
                    headerViewHolder.increaseShareNumber();
                }
            }
        });
        ...
        RxEventBus.subscribe(SubjectCode.SUBJECT_UPDATE_TIMELINE_FAVORITE, this, new Consumer<Object>() {
            ...
            @Override
            public void accept(Object userId) {
                if (userId != null) {
                    headerViewHolder.updateFavorite(Constants.BUZZ_TYPE_IS_FAVORITE);
                }
            }
        });
        ...
        RxEventBus.subscribe(SubjectCode.SUBJECT_UPDATE_TIMELINE_UNFAVORITE, this, new Consumer<Object>() {
            ...
            public void accept(Object userId) {
                if (userId != null) {
                    headerViewHolder.updateFavorite(Constants.BUZZ_TYPE_IS_NOT_FAVORITE);
                }
            }
        });
    }
    ...
    private void onObservable() {
        Disposable listCommentDisposable = listCommentResponseSubject
                ...
                .subscribe(new Consumer<ListCommentResponse>() {
                    @Override
                    public void accept(ListCommentResponse listCommentResponse) {
    ...
    //Header comment listener
    @Override
    public void onDisplayImageDetailScreen(BuzzBean bean, int positionIndexItem, View view) {
        TimelineMediaActivity.launch(CommentActivity.this, view, bean, positionIndexItem);
    }

    @Override
    public void onDisplayLiveStreamScreen(BuzzBean bean, int positionIndexItem, View view) {
        ListBuzzChild buzzChild = bean.listChildBuzzes.get(0);
        if (buzzChild.streamStatus.equals(Constants.LIVE_STREAM_ON))
            (CommentActivity.this).onLiveStreamOption(UserLiveStreamService.Mode.VIEW, bean.buzzId, bean.shareId, bean.listChildBuzzes.get(0).thumbnailUrl);
        else
            TimelineMediaActivity.launch(CommentActivity.this, view, bean, positionIndexItem);
    }

    @Override
    public void onFavorite(BuzzBean bean, View view) {
        String token = UserPreferences.getInstance().getToken();
        if (bean.isFavorite == Constants.BUZZ_TYPE_IS_FAVORITE) {
            ...
            getPresenter().onUnFavorite(removeFavoriteRequest);
        } else {
            AddFavoriteRequest addFavoriteRequest = new AddFavoriteRequest(token,
                    bean.userId);
            getPresenter().onFavorite(addFavoriteRequest);
        }
    }

    @Override
    public void onRemoveStatus(BuzzBean bean, View view) {
        DeleteBuzzRequest deleteBuzzRequest = new DeleteBuzzRequest(UserPreferences.getInstance().getToken(), bean.buzzId);
        getPresenter().onDelete(deleteBuzzRequest);
    }
    ...
    public void onDisplayProfileScreen(BuzzBean bean, View view) {
        if (bean.userId.equals(UserPreferences.getInstance().getUserId()) || typeView == TypeView.TypeViewTimeline.TIMELINE_USER) {
            ...
            MyProfileActivity.launch(CommentActivity.this, view, bean, ActivityResultRequestCode.REQUEST_BUZZ_PROFILE, TypeView.ProfileType.COME_FROM_TIMELINE);
        }
    }

    @Override
    public void onDisplayProfileScreen(String userId, View view) {
        MyProfileActivity.launch(CommentActivity.this, view, userId, TypeView.ProfileType.COME_FROM_TIMELINE_BY_ID);
    }

    @Override
    public void onDisplayTagFriendsScreen(ArrayList<ListTagFriendsBean> listTagFriendsBeans, View view) {
        TagFriendActivity.launch(CommentActivity.this, listTagFriendsBeans, false);
    }

    @Override
    public void onApproval(BuzzBean bean, View view) {
        DialogMaterial.Builder builder = new DialogMaterial.Builder(CommentActivity.this)
                .setStyle(Style.HEADER_WITH_TITLE)
                .setTitle(R.string.common_approved)
                .setContent(R.string.not_approved_buzz)
                .onPositive(R.string.common_ok, new DialogInterface.OnClickListener() {
    ...
    @Override
    public void onDisplayShareAudioPlayScreen(BuzzBean bean, View view) {
        ShareDetailBean shareDetail = bean.shareDetailBean;
        TimelineMediaActivity.launch(CommentActivity.this, view, shareDetail.listChildBuzzes, 0);
    }

    @Override
    public void onDisplayShareLiveStreamScreen(BuzzBean bean, View view) {
        ShareDetailBean shareDetail = bean.shareDetailBean;
        if (shareDetail.listChildBuzzes.get(0).streamStatus.equals(Constants.LIVE_STREAM_ON)) {
            (CommentActivity.this).onLiveStreamOption(UserLiveStreamService.Mode.VIEW,
