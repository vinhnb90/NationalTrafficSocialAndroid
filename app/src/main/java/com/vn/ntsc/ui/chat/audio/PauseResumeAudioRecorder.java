...
//        }
    ...
    public void startRecording() {
        if (currentAudioState.get() == PREPARED_STATE) {
            currentAudioRecordingThread = new AudioRecorderThread(audioFile, MediaRecorder.AudioSource.MIC, sampleRateInHertz, channelConfig, audioEncoding, maxFileSizeInBytes);
    ...
    public long pauseRecording() {
        if (currentAudioState.get() == RECORDING_STATE) {
            ...
            return 1000L - ((maxTimeInMillis - remainingMaxTimeInMillis - 100L) % 1000L);
        } else {
    ...
    /**
     * @return current record duration
     */
    public int getCurrentRecordDuration() {
        // 50 is hardcode to calculate duration correct
        return Math.round((maxTimeInMillis - remainingMaxTimeInMillis - 50) / 1000f);
    }
    ...
    private class AudioRecorderThread extends Thread {
        ...
        public void run() {
            ...
            try {
                ...
                new File(threadAudioFile).mkdirs();
//                throw new RuntimeException("IOException has occurred while recording file: " + threadAudioFile, ex);
                ...
                try {
                    if (dataOutputStream != null) {
                        ...
                        if (!(new File(threadAudioFile).renameTo(new File(threadAudioFile)))) {
