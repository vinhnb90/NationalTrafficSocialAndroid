...
import com.nankai.designlayout.bottomnavigation.notify.Notification;
import com.nankai.designlayout.bottomnavigation.notify.NotificationHelper;
...
public class BottomNavigation extends FrameLayout {
	...
	private void init(Context context, AttributeSet attrs) {
		...
		if (attrs != null) {
			TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.BottomNavigationBehavior_Params, 0, 0);
			try {
				selectedBackgroundVisible = ta.getBoolean(R.styleable.BottomNavigationBehavior_Params_selectedBackgroundVisible, false);
				translucentNavigationEnabled = ta.getBoolean(R.styleable.BottomNavigationBehavior_Params_translucentNavigationEnabled, false);
			} finally {
				ta.recycle();
			}
		}
		...
		// Item colors
		titleColorActive = ContextCompat.getColor(context, R.color.colorBottomNavigationInactive);
		...
		coloredTitleColorActive = ContextCompat.getColor(context, R.color.colorBottomNavigationInactive);
	...
	public void setNotification(Notification notification, int itemPosition) {
        ...
        if (notification == null) {
            notification = new Notification(); // instead of null, use empty notification
        }
